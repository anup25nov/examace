...
-- This script creates tables and functions for end-to-end payment tracking

-- Create payments table with comprehensive status tracking
...
DROP POLICY IF EXISTS "Users can view their own payment verifications" ON public.payment_verifications;
DROP POLICY IF EXISTS "Users can insert their own payment verifications" ON public.payment_verifications;
...
BEGIN
    ...
    INSERT INTO public.payments (
        user_id, payment_id, plan_id, plan_name, amount, 
        payment_method, upi_id, status
    ) VALUES (
        p_user_id, p_payment_id, p_plan_id, p_plan_name, p_amount,
        p_payment_method, p_upi_id, 'pending'
...
-- Admin functions for manual verification and dispute resolution
...
-- Function to get all payments for admin dashboard
...
-- Function to get payment audit trail
CREATE OR REPLACE FUNCTION public.get_payment_audit_trail(p_payment_id TEXT)
RETURNS TABLE(
    action TEXT,
    performed_by UUID,
    old_status TEXT,
...
GRANT EXECUTE ON FUNCTION public.admin_verify_payment(TEXT, TEXT) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_all_payments(TEXT, INTEGER, INTEGER) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_payment_audit_trail(TEXT) TO authenticated;

-- Create a scheduled job to clean up expired payments (optional)
-- This would need to be set up in your Supabase dashboard or using pg_cron
-- SELECT cron.schedule('cleanup-expired-payments', '*/30 * * * *', 'SELECT public.cleanup_expired_payments();');
