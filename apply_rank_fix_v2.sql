-- Apply rank and score fixes directly (Version 2 - with DROP statements)
...
DROP FUNCTION IF EXISTS get_test_rank_and_highest_score(VARCHAR(50), VARCHAR(20), VARCHAR(100), UUID);
DROP FUNCTION IF EXISTS get_user_test_score(UUID, VARCHAR(50), VARCHAR(20), VARCHAR(100));
DROP FUNCTION IF EXISTS upsert_test_attempt(UUID, VARCHAR(50), VARCHAR(20), VARCHAR(100), INTEGER, INTEGER, INTEGER, INTEGER, JSONB, VARCHAR(20));
DROP FUNCTION IF EXISTS update_all_test_ranks(VARCHAR(50), VARCHAR(20), VARCHAR(100));
...
BEGIN
  ...
  -- Get total participants and highest score for this test (always calculate these)
  SELECT 
    COUNT(*)::INTEGER,
    MAX(score)::INTEGER
  ...
  -- If no user score found, return highest score info but null for user-specific data
  IF user_score_val IS NULL THEN
    RETURN QUERY SELECT NULL::INTEGER, total_participants_val, highest_score_val, NULL::INTEGER;
    RETURN;
  END IF;
