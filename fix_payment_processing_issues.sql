...
-- 1. Fix Pro Plus plan not being applied
-- 2. Fix commission processing on successful referral
-- 3. Fix referral network showing pending status
...
-- 2. Create missing referral_commissions table if it doesn't exist
CREATE TABLE IF NOT EXISTS referral_commissions (
    ...
    status varchar(20) DEFAULT 'pending' CHECK (status IN ('pending', 'completed', 'paid', 'cancelled')),
    created_at timestamptz DEFAULT NOW(),
    updated_at timestamptz DEFAULT NOW()
);

-- 3. Update existing pending referral transactions to completed status
...
-- 4. Create commission records for existing completed memberships
INSERT INTO referral_commissions (
    referrer_id,
    referred_id,
    payment_id,
...
-- 5. Update referral_codes table with correct totals
UPDATE referral_codes 
SET 
    total_referrals = (
        SELECT COUNT(*) 
