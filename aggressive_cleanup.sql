-- Aggressive cleanup of all process_referral_commission functions
-- Run this in your Supabase SQL Editor

-- 1. First, let's see what specific names we have
SELECT '=== CURRENT FUNCTION NAMES ===' as step;
SELECT 
    routine_name,
    specific_name,
    routine_definition
...
-- 2. Try to drop with CASCADE and IF EXISTS
SELECT '=== ATTEMPTING DROP WITH CASCADE ===' as step;

-- Drop by specific names with CASCADE
DROP FUNCTION IF EXISTS process_referral_commission_49209 CASCADE;
DROP FUNCTION IF EXISTS process_referral_commission_50962 CASCADE;

-- 3. Try dropping by function name with CASCADE
SELECT '=== DROPPING BY FUNCTION NAME WITH CASCADE ===' as step;
DROP FUNCTION IF EXISTS process_referral_commission CASCADE;

-- 4. Check if any functions remain
...
-- 5. If still remaining, try to drop by specific name again
SELECT '=== FORCE DROP BY SPECIFIC NAME ===' as step;
DO $$ 
DECLARE
    func_record RECORD;
BEGIN
    -- Get all function signatures for process_referral_commission
    FOR func_record IN 
        ...
        AND routine_schema = 'public'
    LOOP
        -- Try to drop each function by its specific name
        BEGIN
            EXECUTE 'DROP FUNCTION ' || func_record.specific_name || ' CASCADE';
            RAISE NOTICE 'Successfully dropped function: %', func_record.specific_name;
        EXCEPTION
            WHEN OTHERS THEN
                RAISE NOTICE 'Failed to drop function %: %', func_record.specific_name, SQLERRM;
        END;
    END LOOP;
END $$;

-- 6. Final check
SELECT '=== FINAL FUNCTION COUNT ===' as step;
SELECT COUNT(*) as remaining_functions
FROM information_schema.routines 
WHERE routine_name = 'process_referral_commission' 
AND routine_schema = 'public';

-- 7. If no functions remain, create the correct one
