-- Fix bulk APIs to match actual test_completions table schema
-- The test_completions table has: topic_id (not topic_name), and doesn't have test_name, is_completed columns
...
BEGIN
  ...
  LEFT JOIN individual_test_scores its ON (
    ...
    AND (its.topic_id = tc.topic_id OR (its.topic_id IS NULL AND tc.topic_id IS NULL))
  )
