-- Fix data type mismatch in bulk APIs - ensure all numeric types are consistent
-- The issue is that COALESCE(its.rank, 0) returns INTEGER but we need to cast it properly
