-- Create bulk APIs for test completions and scores
-- This migration creates efficient bulk APIs to fetch all test data at once
...
BEGIN
  ...
  LEFT JOIN individual_test_scores its ON (
    ...
    AND its.test_id = tc.test_id
    AND (its.topic_id = tc.topic_name OR (its.topic_id IS NULL AND tc.topic_name IS NULL))
  )
  WHERE tc.user_id = user_uuid 
    AND tc.exam_id = exam_name
  ORDER BY tc.test_type, tc.test_name;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public;
